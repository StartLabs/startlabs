...
^{:line 42} (defn job-card [job-info] [:div.thumbnail.job-info [:h2 [:a {:href ^{:line 44} (or ^{:line 44} (linkify ^{:line 44} (:website job-info)) "#")} ^{:line 44} (:company job-info) ":"] [:small " " ^{:line 45} (:position job-info)]] [:div.row-fluid.dateloc [:div.span6 [:i.icon.icon-calendar] ^{:line 48} (:start_date job-info) " — " ^{:line 48} (:end_date job-info)] [:div.span6 [:i.icon.icon-map-marker] ^{:line 49} (:location job-info)]] [:div.row-fluid [:div.description ^{:line 51} (markdownify ^{:line 51} (:description job-info))] [:div.well.well-small "Contact: " [:i.icon.icon-envelope] ^{:line 56} (let [contact-info ^{:line 56} (:contact_info job-info)] [:a {:href ^{:line 58} (linkify contact-info)} contact-info])]]])
^{:line 61} (defn job-list [jobs] [:ul#job-list.thumbnails ^{:line 63} (for [job jobs] [:li.job-brick.span6 {:id ^{:line 64} (:id job)} ^{:line 65} (job-card job)])])