...
(defhtml design-marketing []
  [:div#design-marketing
   [:h1 "Design & Marketing"]
   [:p "Design and marketing are two very valuable aspects of 
    a founding venture. A company must have a product with great 
    design and a plan to market that product effectively. Marketing
    relies on having a well designed product to sell and Design relies
    on having an abled marketing plan to sell it."]
   [:table.table
    [:tbody
     (map #(apply ablank-tr %)
    ...
    [:tbody
     (map #(apply site-twitter %)
          [
           ...
           ["Katie Ray" "http://thehumansideofstartups.wordpress.com/" 
            "ktrae"]
...
(defhtml university-resources []
  [:div#university-resources.clearfix
   [:h1 "University Resources"]
   [:p "A number of universities are beginning to grasp the 
    entrepreneurial fervor. With a rise in entrepreneurship on 
    these campuses, many of these universities are growing a 
    number of resources for their students to help, guide, and advise them."]
...
(defhtml legal []
  [:div#legal
   ...
   [:p "Legally incorporating is quite simple once you get into it –
    ...
    [:a {:href "http://www.goodwinfoundersworkbench.com/"} 
     "Founders Workbench"]
    ...
    [:div.pull-left.span4
     ...
     [:ul (map #(apply ablank-li %)
               [["http://www.startupcompanylawyer.com/" 
                ...
                ["http://freelandbenevich.blogspot.com/" 
                 "Freeland Benevich"]
  ...
  [:div#financing
   ...
   [:ul
    ...
    [:li "Angel Investors - such as "
     [:a {:href "http://blog.jonpierce.com/post/520863618/bostons-best-angel-investors"} "Boston's Best Angel Investors"]]
    [:li "VCs"]]

   [:p "The details of term sheets (the things that you use to issue 
      stock and such) are explained quite well by Brad Feld's "
    [:a {:href "http://www.feld.com/wp/archives/category/term-sheet"}
     "article"]
    ". You should also have a good idea of what your financials of 
     your company should look like – something else that Brad Feld 
     addresses well in another "
    [:a {:href "http://www.feld.com/wp/archives/2011/07/financial-literacy.html"} "article"]"."]])
...
(defhtml incubators []
  [:div#incubators
   ...
   [:p [:strong "Greenhorn Connect"] 
    " is one of the best resources in finding out the when, where, 
        and who of what's going on in Boston. They have great blogs, 
        calendars, resources, and job opportunities."
    ...
    [:tbody
     [:tr
      ...
      [:td [:p
            "YC is one of the most popular incubators in the U.S.
               It is run by Paul Graham since 2005.  It is located in 
               Mountain View, CA and focuses mainly on web startups."]
     ...
     [:tr
      ...
      [:td [:p
            "One of the oldest entrepreneurial competitons around.
               It is run by MIT and thus one member of your team must be from 
               MIT, but they have all sorts of competitions ranging from 
               pitches to business plans. They also have an option for 
               non-MIT students called YouPitch."]
     ...
     [:tr 
      ...
      [:td [:p "A working space run by Polaris Ventures that offers 
              free working spaces for budding entrepreneurs.  
              They have locations in Cambridge, NYC, and San Francisco."]]
      [:td "Free"]]

     [:tr
      [:td [:a {:href "http://cictr.com/"} 
            "Cambridge Innovation Center (CIC)"]]
      [:td [:p "The CIC is a co-working space for startups.
              It is located right in Kendall square and houses nearly 400
              companies. The CIC and it’s venture café are the location 
              for many entrepreneurial events around the Boston area."]]
      [:td "$500/person/month"]]]]])
